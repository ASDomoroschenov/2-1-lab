...
long hash(long id, int table_size) {
...
int add_user(long id, hash_table *table) {
	int exit_code = 0;
	peasant *user = NULL;

	if ((exit_code = create_user(&user, id)) != SUCCESS) {
		return exit_code;
	}
...
peasant *get_user(long id, hash_table table) {
	if (table.size != 0) {
		long hash_value = hash(id, table.size);
